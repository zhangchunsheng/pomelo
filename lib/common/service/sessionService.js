 ...
 * @param uid
 */
SessionService.prototype.isReconnect = function(session, uid) {
    var sessions = this.getByUid(uid);
    if(sessions) {
        if(session.__session__.timestamp - sessions[sessions.length - 1].timestamp < 60 * 1000) {
            session.__session__.reconnect = true;
        } else {
            session.__session__.reconnect = false;
        }
    } else {
        session.__session__.reconnect = false;
    }
